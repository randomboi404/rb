...
const ALLOWED_TYPES = ["A", "AAAA", "CNAME", "MX", "TXT", "URL"];
...
for (const file of files) {
  ...
  for (const r of data.records) {
    const allowedKeys = ["type", "name", "value", "proxied", "priority"];
    ...
    if (!r.name.endsWith(data.subdomain)) {
      fail(`${file}: record outside subdomain`);
    }

    if (typeof r.value !== "string") {
      fail(`${file}: record value must be string`);
    }
  }
}
